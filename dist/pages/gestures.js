...
const asEncId = (n) => {
    if (!Number.isInteger(n) || n < 0 || n > 15)
        throw new Error(`EncId out of range: ${n}`);
    return n;
};
export function GesturePage() {
    // Per-encoder state
    const vals = new Int16Array(16); // 0..127
    const mode = Array(16).fill('standby'); // current mode per enc
    const rec = Array.from({ length: 16 }, () => []); // recorded timelines
    const recT0 = Array(16).fill(0); // ✅ safe up to 2^53
    const playT0 = Array(16).fill(0); // ✅ safe up to 2^53
    ...
    const tickPlayback = (i) => {
        ...
        let idx = 0;
        while (idx < tl.length && tl[idx].t <= t)
            idx++;
    ...
    return {
        ...
        onEvent(ev, ctx) {
            if (ev.type === 'encoder/press' && ev.down) {
                ...
                if (mode[i] === 'standby') {
                    ...
                    finalizeRecording(i); // ✅ add final point
                    beginPlayback(i);
                }
                else { // playback
                    backToStandby(i);
            ...
            if (ev.type === 'encoder/turn') {
                const i = ev.id;
                if (mode[i] === 'playback')
                    return; // ignore deltas while playing
        ...
        render(ctx) {
            if (!dirty)
                return;
            dirty = false;
            const mk = (o = {}) => ({ ring: 0, rgb: 110, ledBrightness: 5, ringBrightness: 31, anim: 'none', ...o });
            const frame = {};
            for (let i = 0; i < 16; i = (i + 1)) {
                const m = mode[i];
                const base = m === 'standby' ? { rgb: COLOR_BLUE, ledBrightness: 5, anim: 'none' } :
