...
import { InputEvent, EncId } from "../core/types.js"
...
export interface InputDecoder {
	pushRaw(msg: RawMsg): void
	onEvent(cb: (ev: InputEvent) => void): void
	setShiftInterceptGlobals(on: boolean): void
}

export function createInputDecoder(): InputDecoder {
	const subs: Array<(e: InputEvent) => void> = []
	const emit = (e: InputEvent) => subs.forEach((cb) => cb(e))
	...
	const inEncRange = (num: number, start: number, count: number) =>
		num >= start && num < start + count
	...
	const ccToDelta = (val: number): number => {
		if (val === 63) return -1
		if (val === 64) return +1
	...
	const pushRaw = (msg: RawMsg) => {
		...
		if (
			...
			const id = (msg.number - enc.ccStart) as EncId
		...
		if (
			...
			const id = (msg.number - btn.noteStart) as EncId
