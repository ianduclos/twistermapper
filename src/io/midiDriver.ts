...
type DeviceMap = typeof deviceMap;

export interface NodeMidiDriverOpts {
  inPort?: number | string;
  outPort?: number | string;
...
export class NodeMidiDriver implements MidiOut, MidiIn {
  private readonly input: midi.Input;
  private readonly output: midi.Output;
  private readonly subscribers: Array<(msg: RawMsg) => void> = [];
  private readonly map: DeviceMap;
  private inPortName = '';
  private outPortName = '';

  constructor(opts: NodeMidiDriverOpts = {}) {
  ...
  setRingBrightness(enc: number, human1_31: number) {
  ...
  // ---- internals ----
  private loadDeviceMap(path: string): DeviceMap {
    const resolved = isAbsolute(path) ? path : resolvePath(process.cwd(), path);
    const json = readFileSync(resolved, 'utf8');
    return JSON.parse(json) as DeviceMap;
  }

  private openPort(io: midi.Input | midi.Output, spec: number | string, kind: 'input' | 'output') {
    const names = this.listPorts(io);
    if (!names.length) throw new Error(`No ${kind} MIDI ports available.`);

    const { index, name } = this.resolvePort(names, spec, kind);
    try {
      io.openPort(index);
    } catch (err) {
      throw new Error(`Unable to open ${kind} port '${name}': ${(err as Error).message}`);
    }
    return { index, name };
  }

  private listPorts(io: midi.Input | midi.Output) {
    const count = io.getPortCount();
    const names: string[] = [];
    for (let i = 0; i < count; i++) names.push(io.getPortName(i));
    return names;
  }

  private resolvePort(names: string[], spec: number | string, kind: 'input' | 'output') {
  ...
  private parseMessage(message: number[]): RawMsg | null {
    ...
    if (typeNibble === 0xB0) return { type: 'cc', channel, number, value };
    if (typeNibble === 0x90 || typeNibble === 0x80) return { type: 'note', channel, number, value };
    return null;
  }

  private sendCc(channel: number, controller: number, value: number) {
    this.sendMessage(0xB0, channel, controller, value);
  }
  private sendNote(channel: number, note: number, velocity: number) {
    this.sendMessage(0x90, channel, note, velocity);
  }
  private sendMessage(status: number, channel: number, data1: number, data2: number) {
...
export interface MidiOut {
  setRing(enc: number, value0_127: number): void;
  setRGB(enc: number, colorIdx1_126: number): void;
  setLedBrightness(enc: number, human0_29: number): void;
  setRingBrightness(enc: number, human1_31: number): void;
  setPulse(enc: number): void;
}
export interface MidiIn {
  onMessage(cb: (msg: RawMsg) => void): void;
}

// Temporary no-op so the app runs without device libs:
export class NullMidi implements MidiOut, MidiIn {
  onMessage() {}
  setRing() {}
  setRGB() {}
  setLedBrightness() {}
